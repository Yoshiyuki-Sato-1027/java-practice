 ...
 *
 * JUnit 5の特徴: - @Test, @BeforeEach, @AfterEach などのアノテーションでテストを構成 -
 * AssertJの流暢なAPI (Fluent API) で読みやすいアサーション - @ParameterizedTestでデータ駆動テストが簡単に書ける
...
class JUnit5BasicsTest {
    ...
    void testStringConcatenation() {
        ...
        assertThat(result).isEqualTo("Hello World").hasSize(11).startsWith("Hello").endsWith("World").contains(" ");
    }
    ...
    void testListOperations() {
        ...
        assertThat(testList).hasSize(3).contains("banana").containsExactly("apple", "banana", "orange")
    ...
    @CsvSource({"1, 1", "2, 4", "3, 9", "5, 25", "10, 100"})
    ...
    @CsvSource({"5, 3, 5", "-5, -3, -3", "7, 7, 7", "0, 5, 5"})
    ...
    void testStringAssertions() {
        ...
        // then: 複数の条件を満たす
        assertAll(() -> assertTrue(text.startsWith("JUnit")), () -> assertTrue(text.endsWith("Framework")),
                () -> assertTrue(text.contains(" ")), () -> assertEquals(17, text.length()),
                () -> assertFalse(text.isEmpty()));

        // AssertJでの検証（より読みやすい）
        assertThat(text).startsWith("JUnit").endsWith("Framework").contains(" ").hasSize(17).isNotEmpty();
    }
    ...
    void testCollectionAssertions() {
        ...
        // then: コレクションの内容を検証
        assertThat(numbers).hasSize(5).containsExactly(1, 2, 3, 4, 5).allMatch(n -> n > 0) // すべての要素が0より大きい
    ...
    void testNonEmptyStrings(String fruit) {
        // then: 文字列が空でないことを検証
        assertThat(fruit).isNotNull().isNotEmpty().isNotBlank();
    }
